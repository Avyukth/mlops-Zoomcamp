...
def perform_grid_search(x_train: pd.DataFrame, y_train: pd.Series) -> Dict[str, GridSearchCV]:
...
def evaluate_model(model, x_train: pd.DataFrame, y_train: pd.Series, x_test: pd.DataFrame, y_test: pd.Series) -> Tuple[float, float]:
    try:
        ...
        mlflow.log_metrics({
            f"{model.__class__.__name__}_precision": report['weighted avg']['precision'],
...
def save_models_and_results(models: List, names: List[str], grid_search_results: Dict[str, GridSearchCV],
                            x_train: pd.DataFrame, y_train: pd.Series, x_test: pd.DataFrame, y_test: pd.Series) -> None:
...
def load_models_and_results(names: List[str]) -> Tuple[List, Dict, pd.DataFrame, pd.DataFrame, pd.Series, pd.Series]:
...
def train_models(data_dir: str) -> Tuple[List, List[str], Dict[str, GridSearchCV], pd.DataFrame, pd.DataFrame, pd.Series, pd.Series]:
    ...
    if y_train.ndim > 1 or y_test.ndim > 1:
        raise ValueError(f"Target variable has incorrect shape. y_train: {y_train.shape}, y_test: {y_test.shape}")
...
def create_performance_plot(models: List, names: List[str], grid_search_results: Dict[str, GridSearchCV],
                            x_train: pd.DataFrame, y_train: pd.Series, x_test: pd.DataFrame, y_test: pd.Series) -> None:
    plt.figure(figsize=(12, 8))
    scores = pd.DataFrame(
        {
            "Model": names,
            "CV Score": [grid_search_results.get(name, {}).get("best_score_", 0.0) for name in names],
...
def main(data_dir: str, rerun_evaluation: bool = False) -> None:
    ...
    with mlflow.start_run(experiment_id=experiment_id) as run:
        if not rerun_evaluation:
            models, names, grid_search_results, x_train, x_test, y_train, y_test = train_models(data_dir)
        else:
