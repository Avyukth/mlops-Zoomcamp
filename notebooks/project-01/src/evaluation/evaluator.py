...
class Evaluator:
    ...
    def evaluate_models(self, models: Dict[str, BaseModel], x_train: pd.DataFrame, y_train: pd.Series, 
                        x_test: pd.DataFrame, y_test: pd.Series) -> Tuple[Dict[str, Dict[str, float]], BaseModel]:
        results = {}
        best_score = 0
        best_model = None
    ...
    def save_best_model(self, model: BaseModel) -> str:
        best_model_path = os.path.join(self.config.MODEL['dir'], "best_model.joblib")
        joblib.dump(model, best_model_path)
        return best_model_path

    def create_performance_plot(self, results: Dict[str, Dict[str, float]]) -> str:
        plot_path = os.path.join(self.config.MODEL['dir'], "model_comparison.png")
